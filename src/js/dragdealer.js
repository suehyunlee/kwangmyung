...
Dragdealer.prototype = {
  ...
  renderHandlePosition: function() {
    ...
    var slides = $('.handle_wrap').children().length;
    var slideWidth = $('.dragdealer').width();
    var slidesWidth = $('.handle_wrap').children().width();
    var min = 0;
    var max = -((slides - 2) * slidesWidth);

    // alert((slides - 2) * slidesWidth);

    var transform = '';
    if (this.options.css3 && StylePrefix.transform) {
      if (this.options.horizontal) {
        if(this.offset.current[0] >= max){
          transform += 'translateX(' + this.offset.current[0] + 'px)';
        } else if(this.offset.current[0] < max) {
